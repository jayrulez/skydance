...
namespace BillBox.Common
{
    ...
    public class UniqueAttribute : ValidationAttribute
    {
        ...
        public UniqueAttribute(Type contextType, string propertyName)
        {
            if (contextType == null)
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            using (var dbcontext = (IDisposable)Activator.CreateInstance(_ContextType))
            {
                ...
                var eSql = string.Format("it.{0} = @{0}", PropertyName);

                var query = (ObjectQuery)methodWhere.Invoke(objectSet,
                  new object[] { eSql, new[] { new ObjectParameter(PropertyName, value) } });
