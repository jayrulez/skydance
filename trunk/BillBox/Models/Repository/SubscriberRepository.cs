    ...
    public class SubscriberRepository: BaseRepository, ISubscriberRepository
    {
        ...
        public IResponse<Subscriber> GetSubscriberPayments(int SubscriberId)
        {
            ...
            try
            {
                using (dbContext)
                {
                    ...
                    {
                        /*trigger call to db to join the payments. if this is not done when the context is closed then the payments will not be accessible*/
                        subscriber.Payments.ToList();
                        response.Result = subscriber;
                    }
                }
            }
        ...
        public IResponse<Subscriber> GetSubscriberPayments(int SubscriberId, int PageNumber, int PageSize)
        {
            ...
            try
            {
                if (PageSize < 1)
                {
                    response.Error = ErrorCode.InvalidPageSize;
                }
                else if (PageNumber < 1)
                {
                    response.Error = ErrorCode.InvalidPageNumber;
                }
                else
                {
                    var subscriber = dbContext.Subscribers.Find(SubscriberId);
        ...
        /// <param name="SubscriberId"></param>
        /// <returns></returns>
        public IResponse<CaptureField> GetSubscriberCapturedFields(int SubscriberId)
        {
            IResponse<CaptureField> response = new Response<CaptureField>();

            try
            {
                using (dbContext)
                {
                    var capturedFields = dbContext.CaptureFields
                        .Where(cf => cf.SubscriberId == SubscriberId)
                        .ToList();

                    if (capturedFields.Count() > 0)
                    {
                        response.Results = capturedFields;
                    }
                    else
                    {
                        response.Error = ErrorCode.SubscriberNotFound;
        ...
        public IResponse<bool> AddSubscriber(Subscriber Subscriber)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    if (ex.Message.Contains(Util.GetAppSetting("UKViolation_SubEmail")))
                    {
                        response.Error = ErrorCode.DuplicateEmailAddress;
                    }
                    else if (ex.Message.Contains(Util.GetAppSetting("UKViolation_SubName")))
                    {
                        response.Error = ErrorCode.DuplicateName;
                    }
                        else if (ex.Message.Contains(Util.GetAppSetting("UKViolation_SubOpName")))
                    {
                            response.Error = ErrorCode.DuplicateOperatingName;
                    }
                    else 
                    {
                        response.Error = ErrorCode.DbError;
        ...
        /// <summary>
        /// Update a Subscriber in the Subscriber Repository and returns a Response object with the Result property set to true on success or false on failure
        /// </summary>
        /// <param name="Subscriber">The Subscriber object to be updated</param>
        /// <returns></returns>
        public IResponse<bool> UpdateSubscriber(Subscriber Subscriber)
        {
            ...
            try
            {
                using (dbContext)
                {
                    ...
                    var Entry = dbContext.Entry(Subscriber).State = System.Data.EntityState.Modified;
